...
- **Résumé de programme sur l'accueil** : avancement global et, surtout, la prochaine séance à enchaîner, sélectionnable d'un clic. Les trois séances d'un programme se suivent en boucle ; une séance abandonnée est reproposée.
...
- `progression/` — Moteur de progression : le barème de paliers de chaque exercice (`paliers.py`), la déduction du niveau atteint à partir de l'historique (`niveaux.py`), l'application des objectifs aux séances (`objectifs.py`) et les programmes sportifs (`programmes.py`).
